...
function friendlyError(error: unknown): string {
  ...
  if (message.includes('MCP_URL_QUERY_FORBIDDEN')) return 'Query Parameters (معاملات الرابط) ممنوعة حتى لا تتسرب Tokens/Secrets (رموز الوصول/الأسرار).'
  if (message.includes('MCP_URL_FRAGMENT_FORBIDDEN')) return 'Fragment (جزء الرابط بعد #) ممنوع في عنوان MCP.'
  if (message.includes('MCP_PRIVATE_NETWORK_FORBIDDEN')) return 'العنوان المحلي/الخاص أو IP المباشر ممنوع في Remote MCP (MCP الخارجي) الحالي.'
  ...
  if (message.includes('MCP_RESPONSE_TOO_LARGE')) return 'رد MCP تجاوز حد الحجم الآمن للهاتف، فأُوقف الاتصال.'
...
export default function McpCenter({ agent, onAgentChange, onNotice }: Props) {
  ...
  function handleKillSwitch(active: boolean) {
    ...
    onNotice(active
      ...
      : 'تم إلغاء Kill Switch. ما زالت Trust (الثقة) وAllowlist (قائمة السماح) والموافقة البشرية مطلوبة.')
  }
  ...
  async function handleDiscover(server: McpServerConfig) {
    setIsBusy(true)
    onNotice('جاري MCP Discovery (اكتشاف أدوات الخادم) عبر HTTPS وبروتوكول 2026-07-28...')
    try {
  ...
  async function executeCall(request: PendingCall, approvedByHuman: boolean) {
    ...
    try {
      ...
      if (result.status === 'approval_required' && !approvedByHuman) {
        setPending(request)
        onNotice('كل MCP Tool Call (استدعاء أداة MCP خارجية) يحتاج Human Approval (موافقة بشرية) قبل الإرسال.')
        return
      }
      ...
      if (result.status === 'success') {
        onNotice('MCP Tool Call نجح بعد Security Gate (بوابة الأمان) والموافقة البشرية، وتكلفته المسجلة 0$.')
      } else {
  ...
  return (
    <section className="card mcp-card">
      ...
      <div className="mcp-warning">
        ...
        <span>إضافة URL لا تعني الثقة. Discovery يحتاج Trust، وكل Tool تبقى Disabled حتى تفعيلها وإضافتها إلى Allowlist. وكل استدعاء خارجي يحتاج موافقتك.</span>
        <div className="approval-actions">
          {killSwitch ? (
            <button className="primary-button" type="button" onClick={() => handleKillSwitch(false)}>إلغاء Kill Switch</button>
          ) : (
            <button className="danger-button" type="button" onClick={() => handleKillSwitch(true)}>⛔ تفعيل Kill Switch</button>
          )}
        </div>
      </div>
          ...
          {tools.map((tool) => {
            ...
            return (
              <article className="mcp-tool" key={tool.name}>
                <div><strong>{tool.name}</strong><p>{tool.description || 'بدون وصف من الخادم.'}</p></div>
                <label>Risk (الخطر)
      ...
      {agent && selectedServer && tools.length > 0 && (
        <div className="mcp-console">
          <label>Tool (الأداة)<select value={selectedToolName} onChange={(event) => setSelectedToolName(event.target.value)}>{tools.map((tool) => <option key={tool.name} value={tool.name}>{tool.name}</option>)}</select></label>
