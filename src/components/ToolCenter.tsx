...
export default function ToolCenter({ agent, onAgentChange, onNotice }: Props) {
  ...
  async function runTool(approvedByHuman: boolean, request?: PendingApproval) {
    ...
    if (result.record.status === 'success') {
      onNotice(`تم تنفيذ Tool (الأداة) محلياً بتكلفة $${result.record.monetaryCostUsd.toFixed(2)}.`)
    } else {
      onNotice(result.record.error || 'تم منع Tool Call (استدعاء الأداة).')
    }
  }

  function denyPending() {
    setPending(null)
    onNotice('تم رفض Human Approval (الموافقة البشرية)، ولم تُنفذ الأداة.')
  }
